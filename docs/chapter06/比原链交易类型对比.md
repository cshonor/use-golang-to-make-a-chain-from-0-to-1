...
**基础结构（TxInput / SpendInput）：**
- `SourceID`: 引用的前一笔交易的哈希（交易ID）
- `SourcePosition`: 引用的输出在交易中的索引位置（从0开始计数）
- `AssetAmount`: 包含资产ID和金额
- `ControlProgram`: 控制脚本（地址），用于验证所有权
- `Arguments`: 见证数据（签名），用于证明有权花费该UTXO

**增强结构（AnnotatedInput - 用于查询和显示）：**

`AnnotatedInput` 是用于查询API和区块浏览器的增强型输入结构，在基础 `TxInput` 的基础上添加了用户友好的信息。
...
- `ControlProgram`: 控制脚本（收款地址），用于锁定输出
- `VMVersion`: 虚拟机版本
- `StateData`: 状态数据（可选）
- `TypedOutput`: 输出类型
  - `OriginalOutputType`: 普通输出
...
**增强结构（AnnotatedOutput - 用于查询和显示）：**

`AnnotatedOutput` 是用于查询API和区块浏览器的增强型输出结构，在基础 `TxOutput` 的基础上添加了用户友好的信息。
...
```json
{
  "inputs": [{
    "spent_output_id": "be08e806d5fa7ba234a586fb8ed68a94c0ff79f657a1c9064db124336acbfa55",
    // 这个 spent_output_id 实际上就是通过 SourceID + SourcePosition 计算出来的
  }],
  "outputs": [
    {"position": 0, "amount": 999999500000, ...},  // 第1个输出
    {"position": 1, "amount": 179800000000, ...},  // 第2个输出
    {"position": 2, "amount": 1000000000000, ...}  // 第3个输出
  ]
}
```
...
因为一个交易可能有多个输出，每个输出都是独立的UTXO：
- 同一个交易的不同输出可能发给不同的人
- 每个输出有不同的金额和地址
- 必须通过 `SourcePosition` 精确定位到具体是哪个输出
